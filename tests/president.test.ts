...
describe("president dashboard", () => {
  it("requires authentication and dashboard access", async () => {
    ...
    const member = await request(app).post("/api/auth/register").send({ email: `president-${Date.now()}@example.com`, password: "vitestpass123", name: "President Test Member", departmentId: "computer-science", gender: "Male", academicLevel: "100 Level" });
    const response = await request(app).get("/api/president/roster").set("Cookie", member.headers["set-cookie"]![0].split(";")[0]);
  ...
  it("returns filtered roster rows and real analytics for the seeded admin", async () => {
    ...
    const roster = await request(app).get("/api/president/roster?academicLevel=100%20Level&limit=2").set("Cookie", cookie);
